...
float normalizar (string qual_norma,
				  string direcao,
				  float input)
{
...
float calcular_Tout(string composto,
					float Tin,
					float calor,
...
{
	cout << "Tin " << Tin << endl;
	float Tin_normalizada = normalizar(composto + "-temp",
									   "original_to_modelo",
									   Tin);

	cout << "Tin normalizada " << Tin_normalizada << endl;

	float enth_atual_normalizada = inferencia(Tin_normalizada, modelo_HxT);

	cout << "Hin normalizada " << enth_atual_normalizada << endl;

	float enth_atual = normalizar(composto + "-enth",
								  "modelo_to_normal",
								  enth_atual_normalizada);

	cout << "Hin " << enth_atual << endl;

	float enth_final = enth_atual + calor;

	cout << "Hout " << enth_final << endl;

	float enth_final_normalizada = normalizar(composto + "-enth",
											  "original_to_modelo",
											  enth_final);

	cout << "Hout normaliazda " << enth_final_normalizada << endl;

	float temp_final_normalizada = inferencia(enth_final_normalizada, modelo_TxH);

	cout << "Tout normalizada " << temp_final_normalizada << endl;

	float Tout = normalizar(composto + "-temp",
							"modelo_to_original",
							temp_final_normalizada);
...
int main() {
	...
	// Teste n-butano 1169,1 kW de troca
	float Tin = 500;
	float Q = -1169.1 / 45;

	float Tout = calcular_Tout("n-butane",
							   Tin,
							   Q,
							   modelos["n-butane"][0],
							   modelos["n-butane"][1]);
